...
import { router, Stack, useNavigation, useFocusEffect } from 'expo-router';
import { getUserHoldings, Holding, cleanupZeroHoldings } from '../services/holdingService';
...
export default function HoldingsScreen() {
  ...
  const fetchHoldings = async () => {
    try {
      ...
      // First, clean up any zero quantity holdings that might still be in the database
      console.log('Cleaning up zero quantity holdings before fetching...');
      const { success, error: cleanupError } = await cleanupZeroHoldings();

      if (cleanupError) {
        console.warn('Error cleaning up zero holdings:', cleanupError);
        // Continue anyway, this is just a cleanup step
      } else {
        console.log('Zero holdings cleanup completed successfully:', success);
      }

      // Now fetch the holdings
      console.log('Fetching holdings after cleanup...');
      ...
      console.log(`Fetched ${holdingsData.length} holdings after cleanup`);
  ...
  // Refresh holdings data whenever the screen comes into focus
  useFocusEffect(
    useCallback(() => {
      console.log('Holdings screen focused - refreshing data');
      fetchHoldings();
      return () => {
